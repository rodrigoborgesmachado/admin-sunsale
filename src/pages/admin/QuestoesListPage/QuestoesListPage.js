...
import './QuestoesListPage.css';
import questoesApi from '../../../services/apiServices/questoesApi';
...
const QuestoesListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await questoesApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await questoesApi.export({ term: term, startDate, endDate });
...
export default QuestoesListPage;
