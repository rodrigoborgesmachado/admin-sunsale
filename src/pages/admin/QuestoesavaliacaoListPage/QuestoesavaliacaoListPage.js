...
import './QuestoesavaliacaoListPage.css';
import questoesavaliacaoApi from '../../../services/apiServices/questoesavaliacaoApi';
...
const QuestoesavaliacaoListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await questoesavaliacaoApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await questoesavaliacaoApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await questoesavaliacaoApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='ID Avaliacao'><span>{item.IdAvaliacao}</span></td>
                        <td data-label='ID Questão'><span>{item.IdQuestao}</span></td>
                        <td data-label='Nota Questão'><span>{item.Notaquestao}</span></td>
...
export default QuestoesavaliacaoListPage;
