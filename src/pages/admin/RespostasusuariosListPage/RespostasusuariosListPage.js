...
import './RespostasusuariosListPage.css';
import respostasusuariosApi from '../../../services/apiServices/respostasusuariosApi';
...
const RespostasusuariosListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await respostasusuariosApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await respostasusuariosApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await respostasusuariosApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
...
export default RespostasusuariosListPage;
