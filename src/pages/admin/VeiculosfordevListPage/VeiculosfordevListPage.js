...
import './VeiculosfordevListPage.css';
import veiculosfordevApi from '../../../services/apiServices/veiculosfordevApi';
...
const VeiculosfordevListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await veiculosfordevApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await veiculosfordevApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await veiculosfordevApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Marca'><span>{item.Marca}</span></td>
                        <td data-label='Modelo'><span>{item.Modelo}</span></td>
                        <td data-label='Ano'><span>{item.Ano}</span></td>
                        <td data-label='Renavam'><span>{item.Renavam}</span></td>
                        <td data-label='Placa do Veículo'><span>{item.Placaveiculo}</span></td>
                        <td data-label='Cor'><span>{item.Cor}</span></td>
...
export default VeiculosfordevListPage;
