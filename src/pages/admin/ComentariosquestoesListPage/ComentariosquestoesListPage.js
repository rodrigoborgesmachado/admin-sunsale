...
import './ComentariosquestoesListPage.css';
import comentariosquestoesApi from '../../../services/apiServices/comentariosquestoesApi';
...
const ComentariosquestoesListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await comentariosquestoesApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await comentariosquestoesApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await comentariosquestoesApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Comentario'><span>{item.Comentario}</span></td>
...
export default ComentariosquestoesListPage;
