...
import './LicencassunsaleproListPage.css';
import licencassunsaleproApi from '../../../services/apiServices/licencassunsaleproApi';
...
const LicencassunsaleproListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await licencassunsaleproApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await licencassunsaleproApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await licencassunsaleproApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Licença'><span>{item.Licenca}</span></td>
...
export default LicencassunsaleproListPage;
