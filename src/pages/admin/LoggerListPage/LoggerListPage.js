...
const LoggerListPage = () => {
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await loggerApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Mensagem'><span>{item.Message}</span></td>
                        <td data-label='AdminId'><span>{item.AdminId}</span></td>
