...
import './ResultadossoletrandoListPage.css';
import resultadossoletrandoApi from '../../../services/apiServices/resultadossoletrandoApi';
...
const ResultadossoletrandoListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await resultadossoletrandoApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await resultadossoletrandoApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await resultadossoletrandoApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Número Acertos'><span>{item.Numeroacertos}</span></td>
...
export default ResultadossoletrandoListPage;
