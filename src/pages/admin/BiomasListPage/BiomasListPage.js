...
import './BiomasListPage.css';
import biomasApi from '../../../services/apiServices/biomasApi';
...
const BiomasListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await biomasApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await biomasApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await biomasApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Distribuição'><span>{item.Distribuicao}</span></td>
                        <td data-label='Características'><span>{item.Caracteristicas}</span></td>
                        <td data-label='Fitofisionomia'><span>{item.Fitofisionomia}</span></td>
