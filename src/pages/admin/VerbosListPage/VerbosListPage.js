...
import './VerbosListPage.css';
import verbosApi from '../../../services/apiServices/verbosApi';
...
const VerbosListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await verbosApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await verbosApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await verbosApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
