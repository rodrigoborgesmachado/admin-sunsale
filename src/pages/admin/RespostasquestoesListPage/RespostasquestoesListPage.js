...
import './RespostasquestoesListPage.css';
import respostasquestoesApi from '../../../services/apiServices/respostasquestoesApi';
...
const RespostasquestoesListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await respostasquestoesApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await respostasquestoesApi.export({ term: term, startDate, endDate });
...
export default RespostasquestoesListPage;
