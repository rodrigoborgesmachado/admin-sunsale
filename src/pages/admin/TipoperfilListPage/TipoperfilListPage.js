...
import './TipoperfilListPage.css';
import tipoperfilApi from '../../../services/apiServices/tipoperfilApi';
...
const TipoperfilListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await tipoperfilApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await tipoperfilApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await tipoperfilApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
...
export default TipoperfilListPage;
