...
import './PostagemListPage.css';
import postagemApi from '../../../services/apiServices/postagemApi';
...
const PostagemListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await postagemApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await postagemApi.export({ term: term, startDate, endDate });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='DescricaoEn'><span>{item.DescricaoEn}</span></td>
                        <td data-label='TipoPostagem'><span>{item.TipoPostagem}</span></td>
                        <td data-label='Capa'><span>{item.Capa}</span></td>
                        <td data-label='Curtidas'><span>{item.Curtidas}</span></td>
                    </tr>
                ))}
                </tbody>
            </table>
...
export default PostagemListPage;
