...
import './UsuarioscrudformsListPage.css';
import usuarioscrudformsApi from '../../../services/apiServices/usuarioscrudformsApi';
...
const UsuarioscrudformsListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await usuarioscrudformsApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await usuarioscrudformsApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await usuarioscrudformsApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Administrador'><span>{item.Administrador}</span></td>
                        <td data-label='Desenvolvedor'><span>{item.Desenvolvedor}</span></td>
                        <td data-label='E-mail'><span>{item.Email}</span></td>
                        <td data-label='Usuariopai'><span>{item.Usuariopai}</span></td>
...
export default UsuarioscrudformsListPage;
