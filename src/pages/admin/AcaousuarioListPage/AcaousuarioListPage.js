...
import './AcaousuarioListPage.css';
import acaousuarioApi from '../../../services/apiServices/acaousuarioApi';
...
const AcaousuarioListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await acaousuarioApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "Usuario" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await acaousuarioApi.export({ term: term, startDate, endDate });
...
export default AcaousuarioListPage;
