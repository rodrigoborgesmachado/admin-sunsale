...
import './CategoriaalimentosListPage.css';
import categoriaalimentosApi from '../../../services/apiServices/categoriaalimentosApi';
...
const CategoriaalimentosListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await categoriaalimentosApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await categoriaalimentosApi.export({ term: term, startDate, endDate });
...
export default CategoriaalimentosListPage;
