...
import './RecuperasenhacrudformsListPage.css';
import recuperasenhacrudformsApi from '../../../services/apiServices/recuperasenhacrudformsApi';
...
const RecuperasenhacrudformsListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await recuperasenhacrudformsApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await recuperasenhacrudformsApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await recuperasenhacrudformsApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='ID Usuário'><span>{item.UserId}</span></td>
...
export default RecuperasenhacrudformsListPage;
