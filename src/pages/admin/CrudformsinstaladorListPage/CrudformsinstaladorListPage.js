...
import './CrudformsinstaladorListPage.css';
import crudformsinstaladorApi from '../../../services/apiServices/crudformsinstaladorApi';
...
const CrudformsinstaladorListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await crudformsinstaladorApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await crudformsinstaladorApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await crudformsinstaladorApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
...
export default CrudformsinstaladorListPage;
