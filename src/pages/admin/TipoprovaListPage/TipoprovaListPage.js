...
import './TipoprovaListPage.css';
import tipoprovaApi from '../../../services/apiServices/tipoprovaApi';
...
const TipoprovaListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await tipoprovaApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await tipoprovaApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await tipoprovaApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
...
export default TipoprovaListPage;
