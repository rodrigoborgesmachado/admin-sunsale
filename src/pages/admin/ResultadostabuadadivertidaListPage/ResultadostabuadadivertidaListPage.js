...
import './ResultadostabuadadivertidaListPage.css';
import resultadostabuadadivertidaApi from '../../../services/apiServices/resultadostabuadadivertidaApi';
...
const ResultadostabuadadivertidaListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await resultadostabuadadivertidaApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await resultadostabuadadivertidaApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await resultadostabuadadivertidaApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
...
export default ResultadostabuadadivertidaListPage;
