...
import './AvaliacaoListPage.css';
import avaliacaoApi from '../../../services/apiServices/avaliacaoApi';
...
const AvaliacaoListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await avaliacaoApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await avaliacaoApi.export({ term: term, startDate, endDate });
    ...
    const updateStatus =  async (isActive,id) => {
        try {
            dispatch(setLoading(true));
            const response = await avaliacaoApi.updateStatus({ status: isActive === 1 ? 'IsDeleted' : 'IsActive', id: id });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Orientação'><span>{item.Orientação}</span></td>
                        <td data-label='Publico?'><span>{item.Ispublic}</span></td>
...
export default AvaliacaoListPage;
